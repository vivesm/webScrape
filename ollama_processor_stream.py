...
class OllamaProcessor:
    """
    Process RAG data using a local Ollama instance with streaming support.
    ...
    def __init__(
        ...
        model: str = "llama2:latest",  # Using llama2:latest as default (more reliable)
        batch_size: int = 10,
    ...
    async def generate_response(self, prompt, max_tokens=500, timeout=20):
        """
        Generate a response using streaming to avoid timeouts.
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(f"{self.ollama_url}/api/generate", json=payload) as response:
                    if response.status != 200:
                        ...
                        logging.error(f"Failed to generate response: {response.status} - {error_text}")
                        return f"Error generating response: HTTP {response.status}"
                    ...
                    async for line in response.content:
                        ...
                        if asyncio.get_event_loop().time() - start_time > timeout:
                            logging.warning(f"Streaming response timed out after {timeout}s")
                            if full_response:
            ...
            logging.warning("Timeout while generating response")
            return "Response generation timed out"
