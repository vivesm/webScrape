#!/usr/bin/env python3
"""
Test Okta Workflows RAG with Ollama

This script tests the RAG capabilities using the Okta Workflows documentation
...
# Sample queries about Okta Workflows
SAMPLE_QUERIES = [
    "What is Okta Workflows?",
    "How do I build flows in Okta Workflows?",
    "What are the execution limits in Okta Workflows?",
    "How do connections work in Okta Workflows?",
    "What are helper flows in Okta Workflows?"
]
...
async def test_rag_response(processor, query, top_k=3):
    ...
    prompt = f"""You are a helpful assistant answering questions about Okta Workflows.
...
async def main():
    ...
    import argparse
    parser = argparse.ArgumentParser(description="Test Okta Workflows RAG with Ollama")
    parser.add_argument("--rag_dir", default="rag_data", help="Directory containing RAG data")
    ...
    if not args.interactive:
        ...
        if use_samples:
            ...
            while True:
                query = input("\nEnter a question about Okta Workflows (or 'exit' to quit): ")
